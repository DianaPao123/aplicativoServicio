...
namespace CertificadorWs
{
    public class CertificadorService : ICertificador
    {
        ...
        public bool ValidarLicencia(string Llave)
        {
            ...
            if (s == null)
            {
                CertificadorService.Logger.Error("Equipo no valido:" + Mac + "  Clave:" + Key);
                return false; }
            else
                return true;
        }
        ...
        public string TimbraCfdi(string comprobante, string userName, string password, string LLave, string aplicacion)
        {
            ...
            try
            {
                ...
                if (comprobante.Contains("<cartaporte:CartaPorte"))
                {
                    ...
                    if (erroIH != "0")
                    {
                      //  if (erroIH.Contains("CP00 - Error en los datos de la carta porte:"))
                     //       CertificadorService.Logger.Error("XML con error Certi: " + comprobante);
