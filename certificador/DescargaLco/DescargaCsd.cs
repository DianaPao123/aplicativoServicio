...
namespace DescargaLco
{
    public class DescargaCsd
    {
        ...
        public bool DescargaFoliosFtp()
        {
            ...
            {
                int indice;
                var rutaIndex = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "IndexCsd.txt");
                indice = File.Exists(rutaIndex) ? int.Parse(File.ReadAllText(rutaIndex)) : 0;
                var rutaFolios = ConfigurationManager.AppSettings["UrlCsd"];
                ...
                int contador = 1;
                var rutaTrabajo = Path.Combine(ConfigurationManager.AppSettings["RutaTrabajo"], "CSD.txt");
                using (var streamRespuesta = respuesta.GetResponseStream())
                {
                    if (streamRespuesta != null)
                    {
                        ...
                        {
                            lector.ReadLine();
                            using (StreamWriter sw = new StreamWriter(rutaTrabajo, false, Encoding.UTF8))
                            {
                ...
                {
                    con.Open();
                    SqlCommand command = new SqlCommand("[InsertaCsd]", con);
